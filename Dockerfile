...
WORKDIR $GOPATH/src/github.com/chongyangshi/forex-exporter/
COPY . .
RUN GO111MODULE=off go get -d -v
RUN GOOS=linux GOARCH=amd64 CGO_ENABLED=0 GO111MODULE=off go build -ldflags="-w -s" -o /go/bin/forex-exporter
